...
public class MoveModifierNextEvent extends ClipEvent {
    @Nullable
    private final UUID modifierId;
    private final int modifierIndex;

    public MoveModifierNextEvent(UUID trackId, UUID clipId, @NotNull UUID modifierId) {
        super(trackId, clipId);
        this.modifierId = modifierId;
        modifierIndex = -1;
    }

    public MoveModifierNextEvent(UUID trackId, UUID clipId, int modifierIndex) {
        super(trackId, clipId);
        modifierId = null;
    ...
    public EventResult apply(@NotNull EventContext<Clip> context) {
        ...
        return modifiers.moveNext(index) ? EventResult.OK : EventResult.NOOP;
    }
}
